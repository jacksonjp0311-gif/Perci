#!/usr/bin/env python3
"""Build Perci Bitwork v3: expanded unique geometry plus weight-resident insights.

V3 preserves the integer-only 4,096-bit associative path. It spends the
sub-200 MiB budget on distinct semantic surfaces and stores a compact concept
table inside the weight header. Nearest prototypes therefore select both an
expert and a governed insight; repeated activations are still deduplicated.
"""
...
def build(output: Path, attempts: int) -> dict:
    ...
    manifest = {
        "name": "Perci Cognitive Weights · Bitwork Transcendence",
        "version": VERSION,
        "format": "PERCIW03",
